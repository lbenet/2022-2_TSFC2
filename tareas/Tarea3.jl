...
# Entonces, si representamos el desarrollo de Taylor de una función ``f(x(t), t)``
...
# la o las variables dependientes en términos de la variable independiente, tales que
# dichas series de Taylor satisfacen la ecuación diferencial.
# - Caso escalar: Implementen la función `coefs_taylor(f, t, u, p)` que calculará los
# coeficientes ``u_{[k]}`` de la expansión de Taylor para la variable dependiente
# en términos de la independiente (``t``), y regresará el desarrollo de Taylor, es decir,
...
# *caso escalar* son `f(u, p, t)`), la variable independiente `t::Taylor`, la variable
# dependiente `u::Taylor`, y `p` que representa cualquier parámetros necesarios que
# se requieran en la función `f`.
...
# y debe estar definida de tal manera que `du` (a la salida) corresponda al lado izquierdo de
# las ecuaciones diferenciales. Es decir, la `du` de entrada será modificada por la función
# de manera apropiada a la salida. La función `coefs_taylor!` debe estar implementada de tal
# manera que `u` y `du` *cambien* (se actualizen) de manera adecuada, es decir, esta función
# cambiará a sus argumentos. (Es por eso que, en este caso, el nombre de la función incluye `!`.)
