...
# que es equivalente al método de Picard. Usaremos inicialmente un ejemplo concreto
...
# *solamente* para un intervalo de $t$ (la variable independiente) en torno al valor *inicial*
# $t_0$, que define a la condición inicial. Esto **no** implica que la solución exista
...
# solución $x(t)$ **no existe** para $t>1/3$, si la condición inicial es $x(0)=3$.

#-
# El comentario anterior implica que,
# si hiciéramos una integración "larga" usando un *paso de integración constante*
# (o sea, a partir de $x(t_k)$ obtenemos $x(t_{k+1})$, con
# $t_{k+1} = t_0 + (k+1)\delta t = t_k + \delta t$, siendo $\delta t$ un valor constante),
# sin importar el método de integración,
