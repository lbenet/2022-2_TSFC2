...
# es un comentario. Para comentar varias líneas usamos *bloques* `#=`
# al inicio y `=#`  al final del comentario.

#= Los nombres de las variables pueden ser, o incluir,
caracteres *unicode*.=#
...
# usando `?` y el nombre de la función

?sin

# ## Vectores y matrices

un_vector = [1, 2, 3, 4] # se definen con `[...]`, y los elementos se separan por `,`

#-
# Noten que la salida del vector incluye información resumida sobre el *tipo* específico
...
# Como se muestra en la siguiente celda, si es necesario *promocionar* los
...
#  Uno puede realizar *ciertas* operaciones entre vectores, donde la dimensión importa
...
# se llama *broadcasting*, lo que resulta en un vector. Esto equivale a hacer la operación de manera
# vectorizada
...
#-
@show(un_vector)
un_vector[2] # 2do elemento del vector; las componentes inician en 1
...
#-
collect(2:4)  # esto hace del rango el vector apropiado; es más eficiente trabajar con rangos
...
#-
#Concateno horizontalmente vectores; noten el espacio entre los vectores columna
una_matriz = [[2, 1, -2] [1, -1, 3] [0, 1, 1]]

#-
#Noten el uso de 2 (o mas) comillas para "cambiar" la dimensión
la_misma_matriz = [2; 1; -2;; 1; -1; 3;; 0; 1; 1]
...
b = un_vector[1:3]  # "sub-vector" de longitud 3, a partir de `un_vector`
...
# `transpose` uno puede efectivamente lograr un producto externo
...
#-
sin(A)  # función `sin` aplicado a la matriz `A` (no corresponde a `sin` de cada elemento!)
...
# cambiados, reasignados, añadidos o borrados, y los de la tupla no. Las matrices,
...
# Otra diferencia entre las tuplas y los vectores es que las operaciones típicas no
...
# Los tipos en Julia corresponden a una estructura (en el sentido de grafos)
# de un árbol. Uno puede conocer los tipos más generales, o más específicos,
...
#-
supertype(Bool) # `Bool` es un subtipo de `Integer` ya que es equivalente a 0 o 1, y requiere 1 bit de memoria
...
# Los tipos pueden ser `concretos` o `abstractos`; la diferencia está en que los tipos
# concretos se pueden representar de una manera *eficiente* en memoria. Los tipos abstractos
