...
# formas de crear nuevos tipos que se acomoden a lo que necesitamos, y algunos trucos para que la
...
# por una u otra razón tienen un significado importante para el problema que lo motiva.
...
#-
# Por cuestiones de eficiencia es conveniente que los tipos de las variables *internas*
# sean concretos; si se requiere aún más
# versatilidad respecto a los tipos internos, se puede definir *tipos parámetricos*,
...
#-
# La propiedad de inmutabilidad no es recursiva; así, si una componente de un tipo
# consiste de algún campo que es mutable (por ejemplo, `Array{T,N}`), entonces
# las componentes individuales de ese campo pueden cambiar.
...
# el caso de constructores internos; en algún sentido estamos devolviendo un objeto
...
#-
p2.x[1] = 6.0  # cambia la primer componente del campo `:x`

#-
p2.x .= [2, 1] # Cambiamos componente a componente (con broadcasting) todo el campo `:x`
...
# diferencia es a la hora de definirlos: debemos usar `mutable struct`.
...
# lo que usar otro tipo de vectores arroja un error.
...
# estructura en memoria de tipos abstractos de antemano, el código será ineficiente. Un ejemplo
...
# Esto es útil porque permite obtener cierta clase de sobrecarga de operadores, y por lo mismo,
...
# El error indica *algo* no relacionado con lo que hemos hecho, sino que
...
# simplemente, no lo podemos visualizar.
...
# A pesar de que **no** hemos sobrecargado la suma (`:+`), ésta funciona gracias a la estructura
# de tipo que hemos impuesto a `MiVector2d{T} <: AbstractArray{T,1}`.

x + y

# Sin embargo, hay que notar que el resultado es un `Array{Float64,1}` y no un
# `MiVector2d{Float64}`. Para logra que el resultado sea del tipo que queremos, debemos sobrecargar
# la función `:+`.
...
# Este ejemplo *no* es uno particularmente interesante, pero muestra que Julia permite adecuar
# las cosas a lo que requerimos, y *extender* las funciones de Julia para que la interacción
