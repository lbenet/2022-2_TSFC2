...
# Lo que aparece *antes* de la función definida arriba, entre comillas triples `"""`,
# son las cadenas de documentación de la función (*docstrings*). Si bien no son
# obligatorias, es altamente recomendable incluir docstrings en el código. Noten,
# además, que hay dos caracteres UTF (no ASCII), `á` e `í`, en el nombre de la función.
...
# esto tiene que ver con que se sobreescribió el único método que había.
...
# variable que la `r` que asignamos trivialmente al principio y cuyo valor *sigue* siendo `1.0`. Los
...
# dentro de una función, esto no se recomienda.
...
# función `push!`, que agrega un elemento al final de un vector.
...
# definiciones son equivalentes y definen la misma función anónima, que corrresponde a
# $f(x) = x^2 + 2x -1$.
...
# Una función anónima, que depende de múltiples argumentos, se escribe por ejemplo
# `(x,y) -> x^2 + y^2`, mientras que una función anónima sin argumentos se escribe
# como `() -> π`.

# ## Tuplas como argumentos y funciones con argumentos variables (`varargs`)
...
# La siguiente ejecución de `lala` distribuye los argumentos de la tupla de entrada; este
# uso de `...` se llama `splat`

lala((1,2,3)...) # equivalente a lala(1,2,3)

# ## Métodos, *multiple dispatch*, y estabilidad de tipo

# Julia permite utilizar el mismo nombre de una función en distintos contextos. Por ejemplo,
...
# Esta multiplicidad del uso de una función (con el mismo nombre) significa que la función
# tiene definidos varios métodos.
...
# devuelva el módulo al cuadrado (`abs2`), en lugar de su cuadrado. En este caso debemos
...
ff(x::Complex) = (@show(x); abs2(x))  # `abs2(z)` devuelve el módulo al cuadrado  de `z`
...
# El hecho de que los distintos métodos que definen a una función se aplican de manera distinta
# *según* el *tipo* de los argumentos de entrada y de su órden es lo que se llama *multiple dispatch*.
# Lograr código rápido en Julia no significa escribir métodos específicos según el tipo
# --aunque a veces esto puede ser útil para especializar el código--, sino que el tipo del
# resultado de una función esté determinado *sólamente* por el tipo de los argumentos de entrada,
# y no por su valor. Es esto lo que se conoce como *estabilidad de tipo*.
...
# Vale la pena notar que en el mensaje de error, está descrita una posible solución para resolver
# la ambigüedad.
...
#
# 1. Llamando a la función que hicieron en el ejercicio anterior `area_vol`, y ejecutando
# `res = area_vol(1.0)`, ¿qué *tipo* tiene `res`?
#
# 1. ¿Qué asignación pueden hacer para separar los resultados de `area_vol`?
#
# 1. ¿Cuál es el tipo de `area_vol`? Hint: ¿Cuál es el tipo de `(area_vol, typeof(area_vol))`?
#
