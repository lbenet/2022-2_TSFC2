...
#=
La variable `glob_i` debe definirse *antes* del `while`;
tiene un caracter *global*.
=#
glob_i = 1
while glob_i <= 5
    println(glob_i)
...
#-
#= Lo siguiente, regresa un `UndefVarError` ya que `v_out` sólo existe dentro
del `while`, o sea, tiene un caracter *local*.
=#
v_out
...
# *local scope*. Por default ese ámbito es local. Entonces, las variables `v_out` no existen fuera
# de ese ámbito, que es la razón del error. Para poderlas usar fuera, entonces,
# debemos definirlas antes, o etiquetarlas con `global`.

# En cuanto a `1:5`, habíamos visto que esto define un `UnitRange{Int64}` que es una manera muy
# conveniente (en memoria) de definir un iterador. Si uno quiere que el iterador no tenga pasos de
# tamaño 1, uno utiliza `1:2:5`, lo que dará saltos de 2 en 2, empezando en 1 y terminando
# en 5.
...
for j = 1:1000
    println(j)
    j ≥ 5 && break # Si `j ≥  5` es verdadero, entonces "ejecuta" `break` interrumpiendo el ciclo `for`.
    jj = j
end
jj
...
# Es mejor usar rangos que vectores, justamente porque así se para evita usar
# memoria. Esto lo podemos ver usando `sizeof`.
...
# 1. Construyan una función que, a partir de un tipo de estructura (e.g., `Int64`), muestre
...
#     - (2) Reemplacen `x` por  `(x+a/x)/2`, que es el promedio de x y a/x.
